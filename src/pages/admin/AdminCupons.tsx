...
const AdminCupons = () => {
  ...
  const [comprovantePreview, setComprovantePreview] = useState<{ url: string; nome: string } | null>(null);
  const [previewDialogOpen, setPreviewDialogOpen] = useState(false);
  const [loadingComprovante, setLoadingComprovante] = useState<Record<string, boolean>>({});
  ...
  const handleConfirmDelete = async () => {
    ...
    const { error } = await supabase.from("cupons").delete().eq("id", deleteId);
    if (error) {
  ...
  const handleViewComprovante = async (comprovanteUrl: string, cupomId: string, nome: string) => {
    try {
      setLoadingComprovante((prev) => ({ ...prev, [cupomId]: true }));
      const filePath = extractFilePathFromUrl(comprovanteUrl);
      if (!filePath) { toast.error("Caminho do arquivo inválido"); return; }
  ...
  const handleDownloadComprovante = async (comprovanteUrl: string, cupomId: string, nome: string) => {
    try {
      setLoadingComprovante((prev) => ({ ...prev, [cupomId]: true }));
      const filePath = extractFilePathFromUrl(comprovanteUrl);
      if (!filePath) { toast.error("Caminho do arquivo inválido"); return; }
  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-foreground">Cupons — Lote Especial</h1>
        <div className="flex gap-2">
          <Button variant="outline" size="icon" onClick={fetchCupons} disabled={loading}>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Cada cupom permite <strong>3 inscrições</strong>. Copie o código criptografado e envie ao comprador via WhatsApp.
      </p>
                ...
                <TableRow key={c.id}>
                    ...
                    {c.comprovante_url ? (
                      <div className="flex items-center justify-center gap-1">
                        <Button
                          ...
                          size="icon"
                          className="h-7 w-7"
                          onClick={() => handleViewComprovante(c.comprovante_url!, c.id, c.nome_titular || "Cupom")}
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-7 w-7"
                          onClick={() => handleDownloadComprovante(c.comprovante_url!, c.id, c.nome_titular || "Cupom")}
                          disabled={loadingComprovante[c.id]}
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(c)} title="Editar cupom">
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleCopy(c.codigo)} title="Copiar código">
                        <Copy className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteClick(c.id)} title="Excluir cupom">
            ...
            <div>
              ...
              {editCupom?.comprovante_url && (
                <Button
                  ...
                  onClick={() => handleViewComprovante(editCupom.comprovante_url!, editCupom.id, editCupom.nome_titular || "Cupom")}
                >
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-4">
            <p className="text-muted-foreground">Tem certeza que deseja excluir este cupom?</p>
